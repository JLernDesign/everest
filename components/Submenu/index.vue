...
// check if menu uses icons
let useIcons = false;
props.data.some((item) => item.icon) && (useIcons = true);

// add filler block if odd number of products
let items = props.data;
let columns = false;
if (useIcons && items.length > 4) {
  items.length % 2 != 0 && items.push("");
  columns = true;
}
