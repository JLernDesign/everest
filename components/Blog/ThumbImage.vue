...
<template>
  <div
    ...
    <!-- blog thumbnail -->
    <DatocmsImage v-if="data.image" :data="data.image.responsiveImage" class="h-full w-full" />

    <!-- media thumbnail -->
    <DatocmsImage v-else-if="data.media?.image" :data="data.media.image.responsiveImage" class="h-full w-full" />

    <!-- live demo thumbnail -->
    <DatocmsImage v-else-if="data.media?.screen && loc == 'live-demo'" :data="data.media.screen.responsiveImage"
      class="h-full w-full" />

    <!-- texture background fallback -->
    <img v-else src="/video/media-bg.jpg" class="absolute left-0 top-0 size-full" />

    <!-- product demo -->
    <div v-if="data.media?.__typename == 'ProductDemoRecord' && loc != 'live-demo'"
      ...
      <div class="relative overflow-hidden rounded-base shadow-media">
        <DatocmsImage v-if="data.media.screen" :data="data.media.screen.responsiveImage" />
        <div class="absolute left-0 top-0 size-full bg-[#2A3440] opacity-80"></div>
      </div>
    </div>

    <!-- has video loop -->
    <div v-if="data.media?.video?.loopClip" class="absolute left-0 top-0 size-full">
    ...
    <template v-if="data.tag?.slug == 'press' && !isVideo">
      ...
      <!-- logo -->
      <div v-if="data.logo" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
    ...
    <!-- collaborations -->
    <div v-if="data.tag?.slug == 'collaborations'"
